...
# attach(credhermes)
# nocliente <-as.numeric(nocliente)
# cpf<-as.numeric(cpf)
# sexo<-as.factor(sexo)
...
# uf<-as.factor(uf)
# cep<-as.factor(cep)
# cep3 <- as.numeric(cep3)
# telefone<-as.numeric(telefone)
# qtd_fixo<-as.numeric(qtd_fixo)
# qtd_movel<-as.numeric(qtd_movel)
# qtd_trab<-as.numeric(qtd_trab)
# qtd_rec<-as.numeric(qtd_rec)
# dt_serasa<-as.Date(dt_serasa,origin="2001-01-01")
# st_hoje<-as.numeric(st_hoje)
# limite_hj<-as.numeric(limite_hj)
# ped_dist<-as.numeric(ped_dist)
# ped_total<-as.numeric(ped_total)
# renda_he<-as.numeric(renda_he)
# melhor_dia<-as.numeric(melhor_dia)
# idade<-as.numeric(idade)
# fx_idade<-as.factor(fx_idade)
# cep<-as.numeric(cep)
# regiao_uf<-as.factor(regiao_uf)
# cidade<-as.factor(cidade)
# perfil<-as.factor(perfil)
# qtd_t<-as.numeric(qtd_t)
# qtd_bom<-as.numeric(qtd_bom)
# qtd_mau<-as.numeric(qtd_mau)
# mx_dt_boleto<-as.Date(mx_dt_boleto,origin="2001-01-01")
# mi_dt_boleto<-as.Date(mi_dt_boleto,origin="2001-01-01")
# mx_dias_atraso<-as.numeric(mx_dias_atraso)
# mi_dias_atraso<-as.numeric(mi_dias_atraso)
# conj<-as.factor(conj)
# qtd_filho<-as.numeric(qtd_filho)
# media_boleto<-as.numeric(media_boleto)
# max_boleto<-as.numeric(max_boleto)
# )
